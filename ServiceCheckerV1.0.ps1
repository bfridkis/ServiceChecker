...
$outputFile = $helpRequest = $QFNInstructionPrinted = $groupByMachineName = $CLOutput1 = $results = $null
...
([string]$args).split('-') | %{ 
                                if ($_.Split(' ')[0] -eq "Services") { $servicesToCheck = $_.Split(' ')[1..$($_.Split(' ').Length - 2)] }
                                elseif ($_.Split(' ')[0] -eq "Comps") { $compsToCheck = $_.Split(' ')[1..$($_.Split(' ').Length - 2)] }
...
if (!$helpRequest) {
    ...
    if (!$groupByMachineName -and $compsToCheck.Count > 1) { 
        do {
    ...
    if ($checkAllServices) { 
        $compsToCheck | ForEach-Object {
            ...
            Catch { $unavailableComps.Add([PSCustomObject]@{'MachineName'=$compToCheck ; "Exception"=$_}) }
        }
    }
    else { 
        $compsToCheck | ForEach-Object {
            $compToCheck = $_
            Try { Get-Service -ComputerName $_ -ErrorAction Stop >$null 
    ...
    elseif ($groupByMachineName -eq "Y") { 
        $outputResults | Select-Object @{n='MachineName' ; e={"$($_.MachineName)"}}, @{n='Service' ; e={"$($_.Service)"}}, @{n='Status' ; e={"$($_.Status)"}} | 
            Group-Object MachineName | 
            Select-Object @{n='Count' ; e={"$($_.Count)"}}, @{n='MachineName' ; e={"$($_.Name)"}}, @{n='Group' ; e={$_.Group | % { "$($_.Service)=$($_.Status)"} | Sort-Object $_.Service}} |
            Sort-Object MachineName -OutVariable Export
    }
    else {
        $outputResults | Select-Object @{n='MachineName' ; e={"$($_.MachineName)"}}, @{n='Service' ; e={"$($_.Service)"}}, @{n='Status' ; e={"$($_.Status)"}} | 
    ...
    if ($outputMode -eq 1 -or $outputMode -eq 3) { 
        ...
        $unavailableComps | Select-Object | Out-File -FilePath $outputFileName -Append
    }
    ...
    if (!$CLOutput1) {
        ...
        Write-Host "`n`n** Unavailable Nodes **"
        $unavailableComps | Select-Object @{n='MachineName' ; e={"$($_.MachineName)"}},  @{n='Exception' ; e={"$($_.Exception)"}}
