...
Write-Output "`n"
Write-Output "`t`t`t`t`t`t`t*%*%*  Service Checker *%*%*`n"
...
if (!$helpRequest) {
    ...
    if (!$servicesToCheck -and !$servicesFilePath -and !$checkAllServices) {
        do {
            $readFileOrManualEntryOrAllServices = Read-Host -prompt "Services to Check:`n`n`tRead Input From File (1) or Manual Entry (2) or All Services (3) [Default = All Services]"
    ...
    elseif ($readFileOrManualEntryOrAllNodes -and $readFileOrManualEntryOrAllNodes -eq 2) { 
        else { Write-Output "`n`rEnter 'f' once finished. Minimum 1 entry. (Enter 'q' to exit.)`n" }
    ...
    $hashedResults = New-Object System.Collections.Generic.List[System.Object]
    $results | ForEach-Object { $hashedResults.Add(@{'Service'=$_.Service ; 'MachineName'=$_.MachineName ; 'Status'=$_.Status})}

    if (@($servicesToCheck).Count -eq 1) {
        $machinesWithoutService = New-Object System.Collections.Generic.List[System.Object]
        $compsToCheck | ForEach-Object { 
            if (!($hashedResults.ContainsValue($_))) { $machinesWithoutService.Add($_) }}
        Write-Output "`n`n`t`t** Machines Without '$servicesToCheck' **`n"
        $machinesWithoutService | Select-Object
        Write-Output "`nTOTAL WITHOUT '$servicesToCheck': $($machinesWithoutService.Count)"
    }
    
    if ($groupByMachineName -eq "Y" -and $outputMode -eq 1) { 
        $results | Select-Object @{n='MachineName' ; e={"$($_.MachineName)"}}, @{n='Service' ; e={"$($_.Name)"}}, @{n='Status' ; e={"$($_.Status)"}} | 
    ...
    if ($outputMode -eq 1 -or $outputMode -eq 3) { $Export | export-CSV -Path $outputFileName -NoTypeInformation }
